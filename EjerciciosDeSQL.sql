...
create table articulos(
 ...
 insert into articulos (codigo, nombre, descripcion, precio,cantidad)
  ...
  select nombre, precio from articulos where precio >= 400;
  select codigo,nombre from articulos where cantidad < 30;
  select descripcion,nombre from articulos where precio <> 100;
...
insert into agenda(apellido, nombre, domicilio, telefono)
  ...
  delete agenda where nombre = 'Juan';
  delete agenda where telefono = '4545454';
 ...
 insert into articulos (codigo, nombre, descripcion, precio,cantidad)
  ...
  delete articulos where precio >= 500 ;
  delete articulos where nombre = 'impresora';
...
insert into medicamentos (codigo,nombre,laboratorio,precio,cantidad)
 ...
 insert into libros (codigo,titulo,autor,editorial)
  ...
  insert into libros (codigo,titulo,autor,editorial)
  values (2,'EL principito','Desconocido','Siri');

  insert into libros (codigo,titulo,autor,editorial)
  values (null,'Aprende Java','Jose Carlos Martinez','Siri');
 ...
 insert into alumnos (legajo,documento,nombre,domicilio)
  ...
  insert into alumnos (legajo,documento,nombre,domicilio)
  values('A568','23545345','Jose Marcos','Avellaneda 348');

  insert into alumnos (legajo,documento,nombre,domicilio)
  values('A568',null,'Jose Marcos','Avellaneda 348');
 ...
 insert into medicamentos (codigo,nombre, laboratorio,precio,cantidad)
  values(4,'Aseptaminofen',500,'Bayer',35,100);
 ...
 insert into peliculas (codigo,titulo,actor,duracion)
  values(21,'Mision imposible 2','Tom Cruise',120);
 ...
 insert into empleados (nombre,documento,sexo,domicilio,sueldobasico,cantidadhijos)
  ...
  values ('Susana Molina','29000555','f','Salta 876',800.888,3);
    ...
    insert into empleados (nombre,documento,sexo,domicilio,sueldobasico,cantidadhijos)
  values ('Susana Molina','29000555','f','Salta 876',80000000.888,3);
 ...
 insert into cuentas
  ...
  values ('25666778','Jose Perez');

  insert into cuentas (numero,documento,nombre,saldo)
  values (5,'28999777','Luis Lopez',34000);

   insert into cuentas(documento,nombre, saldo)
  values (6,'25666778','Jose Perez',45000);
  
  insert into cuentas(documento,nombre, saldo)
  values ('Jose Perez',45000);
...
if object_id ('empleados') is not null
 ...
 select nombre+' '+upper(apellido) as nombre,
 ...
 select nombre,apellido from empleados
  where datepart (month,fechanacimiento)= 10;
 ...
 select nombre,mail,
  ...
  datename(hour,fecha) as hora from visitas order by 3,4,5;

 select mail, pais from visitas
  where datepart(month,fecha)= 10 order by pais;
 ...
 from visitantes where mail is not null and mail <> 'no tiene' group by ciudad;
 ...
 select ciudad,max(montocompra) as maximo
  from visitantes where domicilio is not null and sexo='f'
  group by all ciudad
